...
1. **Side *and* amount sealed in one input proof** — `add8(side).add64(amount)`. Most "confidential"
   ...
   side, in storage, events, or after settlement.
2. **Settlement computed on ciphertext, in the coprocessor** — `payout = FHE.div(FHE.mul(winStake,
   distributable), winPool)`, ACL-gated with `FHE.allow(encPayout, msg.sender)` and moved via
   `confidentialTransfer`, so the payout amount stays encrypted too.
3. **FHE fixed-point overflow guard** — `euint64` multiplication wraps mod 2⁶⁴; large pools route
   through a **Q13 fixed-point ratio** with a bounded precision cost (< winStake / 8192, ≤ 0.013%)
   instead of overflowing. Correct FHE arithmetic under a 64-bit ceiling.
4. **Two cost budgets — the invisible one measured** — a bet is **813k HCU** (~4% of the per-tx cap),
   profiled by our own [fhe-gas-profiler](https://github.com/fhe-profiler/fhe-gas-profiler) and enforced as a
   CI HCU-regression gate. No block explorer measures HCU. See [Cost Profile](#cost-profile).
5. **Live, not scripted** — permissionless `resolveByOracle` (Chainlink) + an autonomous, resumable
   keeper that auto-resolves and auto-reveals within ~30 s. See [Keeper](#keeper).
6. **FHE frontend integration done right** — Vercel Edge relayer proxy, COOP/COEP + CSP for the WASM
   worker's `SharedArrayBuffer`, private payout reveal via EIP-712 `userDecrypt`. See [Frontend](#frontend).

## Architecture

**End-to-end confidential lifecycle** — sealed bid → aggregate reveal → confidential claim:

![Pri-Markets pipeline: encrypt → place bet → accumulate → resolve → reveal → claim](demo/cards/card-01-pipeline.png)

Steps 4–5 (resolve + reveal) are permissionless — an autonomous keeper runs them within ~30 s. `side`, `amount`, and each individual payout stay encrypted end-to-end; only the YES/NO aggregate and a single clearing price ever become public.
...
_Generated by [fhe-gas-profiler](https://github.com/fhe-profiler/fhe-gas-profiler) · @fhevm/solidity 0.11.1 · mock-utils 0.4.2 · solc 0.8.28 · report confidence **LOW**._
